    ...
    if($_SERVER['REQUEST_METHOD']=='POST'){
        ...
        if($password==$cpassword){
            ...
            if($row==1){
                ...
                //In line no 33 & 34inserting direct to table whose name is student. We took input in form of (dd-mm-yyy) but in database it will
                //be stored in the form of (yyyy-mm-dd) No problem!
                $sql="insert into student values('$email','$password','$firstname','$lastname','$gender')";
                $result=mysqli_query($conn,$sql);
                $showdate= "select joindates from student where email='$email'";
                $res=mysqli_query($conn,$showdate);
                $ele=mysqli_fetch_assoc($res);
                // $dateEle = $dateEle['joindates'];
                //Now after inserting we want to display date but if display directly it will show us in the from of (yyyy-mm-dd) because in database it was 
                //stored in the form of (YYYY-mm-dd) only , but we want to display in the from of (dd-mm-yyyy) how we will do?
                //answer : - we will just use the function of changing fromate syntex: date(formate,strtotime(currentdate fromate));
                $newFormat = date("m-d-Y", strtotime($dateEle));   // here we change formate from (yyyy-mm-dd) to (dd-mm-yyyy);
                echo $newFormat;  // display new formate
...
<body>
    <form action="\harendra\createstudent.php" method="POST" >
        <div class="container">
            <div class="logo">
            ...
            <div class="login-both">
                <div class="student">
                    ...
                    <div class="input-box">
                        ...
                        <p>Male <input type="radio" name="gender" id="" value="male"> Female <input type="radio" name="gender" value="female" id=""></p>
                        <!-- <input autocomplete="off" name="joindate" class="inp" type="date" placeholder="date"> -->
                    </div>
